...
function passwordResetEmailHtml(setPasswordUrl) {
  return `
<div style="background:#f8fafc;padding:40px 0;font-family:Arial,sans-serif;">
  <div style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:16px;padding:40px;text-align:center;box-shadow:0 10px 30px rgba(0,0,0,0.06);">
    <div style="font-size:12px;font-weight:700;letter-spacing:2px;color:#6366f1;">DIGITALEYES</div>
    <h2 style="margin-top:12px;font-size:22px;font-weight:900;color:#0f172a;">パスワードの再設定</h2>
    <p style="margin-top:18px;font-size:14px;color:#64748b;line-height:1.6;">パスワード再設定のリクエストがありました。以下のボタンから新しいパスワードを設定してください。</p>
    <p style="margin-top:24px;">
      <a href="${setPasswordUrl}" style="display:inline-block;background:#6366f1;color:#ffffff;font-weight:700;font-size:14px;padding:14px 32px;border-radius:12px;text-decoration:none;box-shadow:0 4px 14px rgba(99,102,241,0.4);">パスワードを再設定する</a>
    </p>
    <p style="margin-top:24px;font-size:13px;color:#94a3b8;">このリンクの有効期限は1時間です。心当たりがない場合はこのメールを無視してください。</p>
    <hr style="border:none;border-top:1px solid #f1f5f9;margin:32px 0;">
    <p style="font-size:12px;color:#94a3b8;">SEO Scan by DIGITALEYES</p>
  </div>
</div>
...
async function sendPasswordResetEmail({ to, setPasswordUrl }) {
  const sesFrom = process.env.SES_FROM;
  const emailUser = process.env.EMAIL_USER;
  const emailPass = process.env.EMAIL_PASS;
  const html = passwordResetEmailHtml(setPasswordUrl);
  ...
  if (sesFrom) {
    const command = new SendEmailCommand({
      ...
      Destination: { ToAddresses: [to] },
      Message: {
        Subject: { Data: "SEO Scan パスワードの再設定", Charset: "UTF-8" },
        Body: {
          Text: { Data: `SEO Scan パスワード再設定: ${setPasswordUrl}`, Charset: "UTF-8" },
          Html: { Data: html, Charset: "UTF-8" }
        }
      }
    });
    try {
      ...
      console.error("[SES] パスワード再設定メール送信失敗:", e.message);
      return false;
    }
  }
  if (emailUser && emailPass) {
    ...
    try {
      await transporter.sendMail({
        from: `"SEO Scan" <${emailUser}>`,
        to,
        subject: "SEO Scan パスワードの再設定",
        text: `SEO Scan パスワード再設定: ${setPasswordUrl}`,
        html
      });
      return true;
    } catch (e) {
      console.error("[メール] パスワード再設定送信失敗:", e.message);
      return false;
    }
  }
  console.log("[DEV] パスワード再設定URL:", setPasswordUrl);
  return true;
}
